...
public class DataBlocker extends BaseDataProcessor {
    ...
    public Data process(DataStartSignal signal) throws DataProcessingException {
        if (curInBufferSize < blockSizeSamples || curInBufferSize == 0) {
            sampleRate = signal.getSampleRate();
            blockSizeSamples =
                    (int) Math.round(sampleRate * blockSizeMs / 1000);
            curInBufferSize = 0;
            curFirstSamplePos = 0;
            inBuffer.clear();
            return null;
    ...
    public Data process(DoubleData data) throws DataProcessingException {
        if (curInBufferSize < blockSizeSamples || curInBufferSize == 0) {
            inBuffer.add(data);
            curInBufferSize += data.getValues().length;
            return null;
        }
        ...
        int copiedSamples = 0;
        long firstSample =
                inBuffer.get(0).getFirstSampleNumber() + curFirstSamplePos;

        while (!inBuffer.isEmpty()) {
            DoubleData dd = inBuffer.remove(0);
            double[] values = dd.getValues();
            int copyLength =
                    Math.min(blockSizeSamples - copiedSamples, values.length
                            - curFirstSamplePos);

            System.arraycopy(values, curFirstSamplePos, newSampleBlock,
                    copiedSamples, copyLength);

            // does the current data-object contains more samples than necessary? -> keep the rest
            // for the next block
        ...
        curInBufferSize =
                inBuffer.isEmpty() ? 0 : inBuffer.get(0).getValues().length
                        - curFirstSamplePos;
